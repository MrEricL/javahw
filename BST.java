...
public class BST 
{
	...
	while (leader.getValue()!=remVal){
	    ...
	    follower=leader;
	    if (leader.getValue()>remVal) leader=leader.getLeft();
	    else leader.getRight();
	...
	if ( isLeaf(leader) ) {
	    //subcase: 1-node tree
	    if (leader.getValue()==_root.getValue())  _root=null;
	    //subcase: removal node is a left child
	    if (follower.getLeft().getValue()==leader.getValue()) follower.setLeft(null);
	...
	//action: replace node with only child
	    ...
	    if (leader.getValue()==root.getValue()) _root=leader.getLeft();
	    //subcase: removal node is a left child
	    if (leader.getValue()==follower.getLeft().getValue()) follower.setLeft(leader.getLeft());
	    //subcase: removal node is a right child
	    else follower.setRight(leader.getLeft())
	}
	    ...
	    //subcase: removal node is root
	    if (leader.getValue() == _root.getValue())	_root = (leader.getRight());
	    //subcase: removal node is a left child
	    if (leader.getValue() == follower.getLeft().getValue()) follower.setLeft(leader.getRight());
	    //subcase: removal node is a right child
	    else follower.setRight(leader.getRight());
	...
	//        promoting its left child if exists
	    ...
	    //subcase: removal node is root
	    if (leader.getValue() == _root.getValue()) _root.setValue(tmp.getValue());
	    //subcase: removal node is a left child
	    if (leader.getValue() < follower.getValue()) follower.setLeft(tmp);
	    //subcase: removal node is a right child
	    else follower.setRight(tmp);
